...
function maintenance(){
 ...
 `<div class="maintenance-tabs">${filterTabs.map(tab=>`<button class="${currentFilter===tab.key?'active':''}" onclick="setFilter('${tab.key}')"><span>${tab.label}</span><b>${tab.count}</b></button>`).join('')}</div>
   <section class="card maintenance-list-card">
     ...
     <div class="maintenance-list-footer">Showing ${sortedItems.length} of ${maintenance.length} items</div>
   </section>
   </div>
...
window.openSettingsPage=async function(){setProfileMenu(false);current='Settings';currentFilter='All';try{await refreshSettingsData()}catch(error){toast(error.message||'Unable to load Settings')}render()}
