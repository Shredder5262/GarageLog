...
function mobileReceiptReviewMarkup(doc,index){
 ...
 const ocr=doc.receiptOcr||{},captured=parseDocumentDate(doc.capturedUtc||doc.addedAt),date=String(ocr.purchaseDate||captured?.toISOString().slice(0,10)||new Date().toISOString().slice(0,10)),merchant=String(ocr.merchant||doc.shop||''),amount=ocr.amount??'',gallons=ocr.gallons??'',price=ocr.pricePerGallon;
 return `<section class="mobile-receipt-review"><div class="mobile-receipt-review-heading"><div><span class="wizard-eyebrow">MOBILE RECEIPT · PENDING REVIEW</span><strong>Verify the OCR values before creating the fuel expense.</strong></div><span class="mobile-receipt-pending">Pending</span></div><div class="mobile-receipt-review-grid"><label>Station / Merchant<input name="Mobile Receipt Merchant" value="${esc(merchant)}" placeholder="Fuel station"></label><label>Purchase Date<input name="Mobile Receipt Date" type="date" value="${esc(date)}"></label><label>Total Amount<input name="Mobile Receipt Amount" type="number" min="0" step="0.01" value="${esc(amount)}" placeholder="0.00"></label><label>Gallons<input name="Mobile Receipt Gallons" type="number" min="0" step="0.001" value="${esc(gallons)}" placeholder="0.000"></label></div><div class="mobile-receipt-review-meta"><span>OCR: <strong>${esc(documentOcrStatusLabel(doc))}</strong></span>${price?`<span>Detected price/gal: <strong>$${Number(price).toFixed(3)}</strong></span>`:''}<label><input name="Mobile Receipt Full Tank" type="checkbox"> Full-tank fill-up</label><button type="button" class="primary" onclick="approveMobileReceipt(${index})">Approve & Create Fuel Expense</button></div></section>`
}
