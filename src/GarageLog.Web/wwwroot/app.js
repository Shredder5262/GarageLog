...
function documentExpenseLinkMarkup(doc,index){
  ...
  return `<section class="document-linked-expense full" data-document-context="Receipts,Registration,Insurance,Warranties,Other"><div class="document-expense-heading"><span class="wizard-eyebrow">EXPENSE LINK</span><p>${linked?'This document is linked to an expense. You can change or remove the relationship here.':'This document is not currently represented in Expenses.'}</p></div><div class="document-upload-details-grid"><label class="full">Linked Expense<select name="Linked Expense">${documentExpenseLinkOptions(doc,index)}</select><small class="wizard-field-help">Choose an existing unlinked expense, or leave this as No linked expense.</small></label></div><div class="modal-actions">${linkedIndex>=0?`<button type="button" class="secondary" onclick="editLinkedExpenseFromDocument(${index})">${svg('edit')} Edit Linked Expense</button>`:`<button type="button" class="secondary" onclick="createExpenseFromDocument(${index})">${svg('dollar')} Create Expense</button>`}</div></section>`
}
function documentEditFieldsHtml(doc={},index=null){
  const category=normalizeDocumentCategory(doc.category||'Receipts'),services=serviceValuesText(documentServiceValues(doc)),shop=documentShopValue(doc),relevantDate=doc.relevantDate||doc.startsOn||'';
  return `<label class="full">File Name<input name="File Name" type="text" value="${esc(doc.name||'')}" required></label><label>Category<select name="Category" required>${documentCategoryOptions(category)}</select></label><label><span class="document-relevant-date-label">${esc(documentRelevantDateLabel(category))}</span><input name="Relevant Date" type="date" value="${esc(relevantDate)}"><small class="wizard-field-help">Date this document represents. Used on the vehicle timeline; Date Added remains unchanged.</small></label><label class="full document-context-field" data-document-context="Receipts">Service<input name="Service" type="text" value="${esc(services)}" placeholder="Oil Change, Tire Rotation, Air Filter"><small class="wizard-field-help">Separate multiple services with commas.</small></label><label class="full document-context-field" data-document-context="Receipts">Shop<input name="Shop" type="text" value="${esc(shop)}" placeholder="Company or shop that performed the service"></label><label class="document-context-field" data-document-context="Registration,Insurance,Warranties">Expiration Date<input name="Expiration Date" type="date" value="${esc(doc.expiresOn||'')}"></label><label class="full">Tags<input name="Tags" type="text" value="${esc((doc.tags||documentTags(doc)).join(', '))}" placeholder="oil, receipt, 2026"></label>${documentExpenseLinkMarkup(doc,index)}`
}
...
function documentCoverageDates(doc){const start=parseDocumentDate(doc?.relevantDate||doc?.startsOn),expiry=parseDocumentDate(doc?.expiresOn);return{start,expiry,sortDate:expiry||start||null}}
