...
function syncDocumentExpenseSuggestions(){
  const category=document.querySelector('#modalFields [name="Category"]'),expenseCategory=document.querySelector('#modalFields [name="Expense Category"]');
...
function syncDocumentContextFields(){
  ...
  const category=normalizeDocumentCategory(categoryControl.value),isReceipt=category==='Receipts';
  ...
  if(linkedSection)linkedSection.hidden=!isReceipt;
  if(checkbox){if(!isReceipt)checkbox.checked=false;else if(!checkbox.dataset.userChanged)checkbox.checked=true}
...
function documentMatchesSearch(doc,value=documentSearchQuery){
 ...
 const normalizedCategory=normalizeDocumentCategory(doc?.category);
  return [doc?.name,doc?.originalName,normalizedCategory,documentShopValue(doc),...documentServiceValues(doc),vehicleNameFromId(doc?.vehicleId||state.activeVehicleId),doc?.ocrText,...documentTags(doc||{})].filter(Boolean).join(' ').toLowerCase().includes(query)
}
 ...
 if(type==='mileage'){state.mileage=Number(fd.get('Mileage'));state.mileageHistory.push({date:new Date().toISOString(),mileage:state.mileage,source:fd.get('Source')});persistActiveVehicle()}
   ...
   if(existing){
    ...
    if(isEdit){
      ...
      const createExpense=category==='Receipts'&&fd.get('Create Expense')==='on',expenseAmount=Number(fd.get('Expense Amount')||0);if(createExpense&&(!(expenseAmount>0)))throw new Error('Enter an expense amount greater than zero.');
      ...
      await saveNow();await indexDocumentRecord(0,{quiet:true})
    }
  }
