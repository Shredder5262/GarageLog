...
function initialSetupDefaults(){return{step:1,type:'Car',powertrain:'Gasoline / Internal Combustion',year:'',make:'',model:'',trim:'',vin:'',engine:'',drivetrain:'',color:'',mileage:'0',mileageDate:new Date().toISOString().slice(0,10),localAlerts:true,emailEnabled:false,odometerReminder:true,registrationDate:'',insuranceDate:''}}
...
function firstRunVehicleDetailsFields(s){
 ...
 return `<div class="first-run-form-grid">
  ...
  ${isTrailer?'':`<label>Engine / Motor <span>Optional</span><input data-first-run-field="engine" maxlength="100" value="${esc(s.engine)}" placeholder="Engine or motor"></label><label>${drivetrainLabel} <span>Optional</span><input data-first-run-field="drivetrain" maxlength="100" value="${esc(s.drivetrain)}" placeholder="${s.type==='Motorcycle'?'Belt, chain, shaft, etc.':'FWD, RWD, AWD, 4WD, etc.'}"></label>`}
 </div>`
}
function firstRunStepContent(s){
 ...
 const identityDetails=[s.color?`Color: ${s.color}`:'',s.vin?`VIN: ${s.vin}`:'',s.type!=='Trailer'&&s.engine?`Engine: ${s.engine}`:'',s.type!=='Trailer'&&s.drivetrain?`${s.type==='Motorcycle'?'Final drive':'Drivetrain'}: ${s.drivetrain}`:''].filter(Boolean);
...
function reports(){
 ...
 const sum=list=>list.reduce((total,item)=>total+Number(item?.amount||0),0);
 const totalSpend=sum(scopedExpenses),maintenanceExpenseCategories=['Maintenance','Repair','Parts'],trackedMaintenance=activeMaintenance();
 ...
 return `<div class="reports-page-grid">
  <div class="reports-main-column">
   ...
   <div class="reports-chart-grid">
    ...
    <section class="card reports-chart-card maintenance-breakdown-card"><div class="section-title"><h3>Maintenance Cost Breakdown</h3><strong class="chart-summary">${trackedMaintenance.length} maintenance item${trackedMaintenance.length===1?'':'s'} tracked · ${maintenanceExpenseCount} expense${maintenanceExpenseCount===1?'':'s'} in range</strong></div>${maintenanceHtml}</section>
 ...
 if(type==='mileage'){state.mileage=Number(fd.get('Mileage'));state.mileageHistory.push({date:new Date().toISOString(),mileage:state.mileage,source:fd.get('Source')});persistActiveVehicle()}
   ...
   index!==null?state.expenses[index]=x:state.expenses.unshift(x)
    ...
    if(isEdit){
      ...
      const createExpense=documentCategorySupportsLinkedExpense(category)&&fd.get('Create Expense')==='on',expenseAmount=Number(fd.get('Expense Amount')||0);if(createExpense&&(!(expenseAmount>0)))throw new Error('Enter an expense amount greater than zero.');
