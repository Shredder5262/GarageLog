...
const navItems=['Dashboard','Garage','Maintenance','Expenses','Documents','Reminders','Reports'];
...
async function loadState(){
 ...
 state=await r.json();const migrated=normalizeState();if(migrated&&canWrite())await saveNow()
}
...
function normalizeState(){
 let migrated=false;
 const defaults={year:'',make:'',model:'',name:'',trim:'',type:'Car',powertrain:'Gasoline / Internal Combustion',lifecycleStatus:'Active',engine:'',transmission:'',vin:'',drivetrain:'',color:'',imageStoredName:null};
 ...
 state.vehicles=state.vehicles.map((vehicle,index)=>{const v={...defaults,...vehicle};if(!v.id){v.id=makeVehicleId();migrated=true}v.id=String(v.id);v.name=v.name||[v.year,v.make,v.model].filter(Boolean).join(' ')||`Vehicle ${index+1}`;v.type=v.type||inferVehicleType(v);v.lifecycleStatus=['Active','Sold','Decommissioned'].includes(v.lifecycleStatus)?v.lifecycleStatus:'Active';v.mileage=Number((v.mileage??(index===0?state.mileage:0))||0);v.mileageHistory=Array.isArray(v.mileageHistory)&&v.mileageHistory.length?v.mileageHistory:[{date:new Date().toISOString(),mileage:v.mileage,source:'Initial record'}];v.metrics={averageMpg:0,...(v.metrics||{})};return v});
...
function initialSetupDefaults(){return{step:1,type:'Car',powertrain:'Gasoline / Internal Combustion',year:'',make:'',model:'',trim:'',mileage:'0',mileageDate:new Date().toISOString().slice(0,10),localAlerts:true,emailEnabled:false,odometerReminder:true,registrationDate:'',insuranceDate:''}}
...
function firstRunStepContent(s){
 ...
 if(s.step===2)return `<div class="first-run-form-step"><span class="wizard-eyebrow">VEHICLE DETAILS</span><h1>Add your first vehicle</h1><p>These values can be changed later from My Garage.</p><div class="first-run-form-grid"><label>Vehicle type<select data-first-run-field="type">${['Car','Truck','Motorcycle','Trailer'].map(value=>`<option ${s.type===value?'selected':''}>${value}</option>`).join('')}</select></label><label>Powertrain<select data-first-run-field="powertrain">${VEHICLE_POWERTRAINS.map(value=>`<option ${s.powertrain===value?'selected':''}>${value}</option>`).join('')}</select></label><label>Year<input data-first-run-field="year" inputmode="numeric" maxlength="4" value="${esc(s.year)}" placeholder="2020"></label><label>Make<input data-first-run-field="make" maxlength="50" value="${esc(s.make)}" placeholder="Vehicle make" required></label><label>Model<input data-first-run-field="model" maxlength="50" value="${esc(s.model)}" placeholder="Vehicle model" required></label><label>Trim <span>Optional</span><input data-first-run-field="trim" maxlength="60" value="${esc(s.trim)}" placeholder="Optional trim"></label></div></div>`;
...
function syncDocumentExpenseSuggestions(){
 const category=document.querySelector('#modalFields [name="Category"]'),displayName=document.querySelector('#modalFields [name="Display Name"]');
 const expenseCategory=document.querySelector('#modalFields [name="Expense Category"]'),notes=document.querySelector('#modalFields [name="Expense Notes"]');
 ...
 if(notes&&!notes.dataset.userChanged){const label=String(displayName?.value||document.getElementById('documentUploadSelectedName')?.textContent||'').trim();notes.value=label==='No file selected'?'':label.replace(/\.[^.]+$/,'')}
}
function documentUploadFieldsHtml(){return `<div class="document-upload-workspace full"><section class="document-upload-preview-panel"><label class="document-upload-file-picker">Choose document<input id="documentUploadFile" name="File" type="file" required></label><div class="document-upload-selected"><span id="documentUploadSelectedIcon">${fileTypeIcon({name:'file'})}</span><div><strong id="documentUploadSelectedName">No file selected</strong><small id="documentUploadSelectedMeta">Choose a file to begin</small></div></div><div id="documentUploadPreviewBody" class="document-upload-preview-body">${documentUploadEmptyPreview()}</div></section><aside class="document-upload-details-panel"><div class="document-upload-details-heading"><span class="wizard-eyebrow">DOCUMENT DETAILS</span><h4>Organize this file</h4><p>These details control how the document appears and can be searched later.</p></div><div class="document-upload-details-grid"><label>Category<select name="Category" required>${['Receipts','Registration','Insurance','Manuals','Warranties','Photos','Other'].map(value=>`<option>${value}</option>`).join('')}</select></label><label>Custom Folder<select name="Custom Folder" required>${['No custom folder',...state.documentFolders.map(folder=>folder.name)].map(value=>`<option>${esc(value)}</option>`).join('')}</select></label><label class="full">Display Name<input name="Display Name" type="text" placeholder="Uses the original filename by default"></label><label class="full">Tags<input name="Tags" type="text" placeholder="oil, receipt, 2026"></label><label>Expiration Date<input name="Expiration Date" type="date"></label></div><section class="document-linked-expense"><label class="linked-expense-toggle"><input id="documentCreateExpense" name="Create Expense" type="checkbox"><span>${svg('dollar')}</span><span><strong>Create linked expense?</strong><small>Save an expense record with this document and keep the two entries associated.</small></span></label><div id="documentExpenseFields" class="document-expense-fields" hidden><div class="document-expense-heading"><span class="wizard-eyebrow">EXPENSE DETAILS</span><p>These values will be added to the active vehicle's Expenses page.</p></div><div class="document-upload-details-grid"><label>Expense Date<input name="Expense Date" type="date" value="${new Date().toISOString().slice(0,10)}"><small class="wizard-field-help">Used in expense charts and reports.</small></label><label>Category<select name="Expense Category">${['Maintenance','Repair','Fuel','Parts','Insurance','Registration','Other'].map(value=>`<option>${value}</option>`).join('')}</select></label><label>Vendor<input name="Expense Vendor" type="text" placeholder="Shop or seller"></label><label>Amount<input name="Expense Amount" type="number" min="0" step="0.01" placeholder="0.00"></label><label class="full">Notes<input name="Expense Notes" type="text" placeholder="What was purchased or serviced?"></label></div><div class="linked-expense-note">The uploaded file will remain available from Documents, and the expense will appear in ownership reports.</div></div></section><div class="upload-note">Files are saved under the local <code>data/documents</code> folder and are never sent to an external service.</div></aside></div>`}
function initializeDocumentUploadDialog(){const file=document.getElementById('documentUploadFile');if(!file)return;['Category','Display Name','Tags'].forEach(name=>{const control=document.querySelector(`#modalFields [name="${name}"]`);if(control)control.addEventListener(name==='Category'?'change':'input',()=>{control.dataset.userChanged='true';syncDocumentExpenseSuggestions()})});['Expense Category','Expense Notes'].forEach(name=>{const control=document.querySelector(`#modalFields [name="${name}"]`);if(control)control.addEventListener(name==='Expense Category'?'change':'input',()=>control.dataset.userChanged='true')});const expenseToggle=document.getElementById('documentCreateExpense');if(expenseToggle)expenseToggle.addEventListener('change',()=>{toggleDocumentExpensePanel(undefined,{reveal:true});syncDocumentExpenseSuggestions()});file.addEventListener('change',()=>{updateDocumentUploadPreview(file.files?.[0]);syncDocumentExpenseSuggestions()});toggleDocumentExpensePanel(false);syncDocumentExpenseSuggestions()}
...
function estimateDocumentExpiry(doc){const category=normalizeDocumentCategory(doc?.category);const date=parseDocumentDate(doc?.expiresOn||doc?.date||doc?.addedAt);if(!date)return null;const expiry=new Date(date.getTime());if(category==='Registration')expiry.setFullYear(expiry.getFullYear()+1);else if(category==='Insurance')expiry.setMonth(expiry.getMonth()+6);else if(category==='Warranties')expiry.setFullYear(expiry.getFullYear()+1);else return null;return expiry}
function documentFolderById(id){return state.documentFolders.find(folder=>folder.id===id)||null}
function documentAction(action){if(action==='manageFolders')return openDocumentFolderManager();if(action==='share')return openDocumentShare();if(action==='export')return openDocumentSelection('export');if(action==='print')return openDocumentSelection('print');if(action==='manageStorage')return openDocumentStorage();toast('Document action unavailable')}
...
window.openDocumentPreview=function(index){const doc=state.documents[index];if(!doc)return;doc.lastViewedAt=new Date().toISOString();save('Document opened');const dialog=ensureDynamicDialog('documentPreviewDialog','document-preview-dialog'),folder=documentFolderById(doc.folderId);dialog.innerHTML=`<div class="modal-header"><div class="document-preview-title">${fileTypeIcon(doc)}<div><h3>${esc(doc.name)}</h3><p>${esc(normalizeDocumentCategory(doc.category))}${folder?` · ${esc(folder.name)}`:''} · ${esc(doc.size||formatBytes(doc.bytes))}</p></div></div><button type="button" class="icon-btn document-preview-close">${svg('close')}</button></div><div class="document-preview-body">${documentPreviewMarkup(doc)}</div><div class="document-preview-footer"><div class="document-preview-tags">${documentTags(doc).map(tag=>`<span>${esc(tag)}</span>`).join('')}</div><div class="modal-actions"><a class="secondary" href="/api/documents/${encodeURIComponent(doc.storedName||'')}" download="${esc(doc.name)}">${svg('download')} Download</a><button type="button" class="secondary document-preview-print">${svg('printer')} Print</button><button type="button" class="primary document-preview-done">Close</button></div></div>`;dialog.querySelector('.document-preview-close').onclick=()=>dialog.close();dialog.querySelector('.document-preview-done').onclick=()=>dialog.close();dialog.querySelector('.document-preview-print').onclick=()=>printDocument(index);dialog.showModal()}
...
function openDocumentFolderManager(){const dialog=ensureDynamicDialog('documentFolderDialog','document-folder-dialog');const draw=()=>{dialog.innerHTML=`<div class="modal-header"><div><span class="wizard-eyebrow">DOCUMENT ORGANIZATION</span><h3>Manage Folders</h3><p>Create up to 10 custom folder filters.</p></div><button type="button" class="icon-btn folder-close">${svg('close')}</button></div><div class="folder-manager-body"><form class="folder-create-form"><label>New folder name<input name="folderName" maxlength="32" placeholder="Repair invoices" ${state.documentFolders.length>=10?'disabled':''}></label><button type="submit" class="primary" ${state.documentFolders.length>=10?'disabled':''}>${svg('plus')} Add Folder</button></form><div class="folder-limit-line"><span>${state.documentFolders.length} of 10 custom folders</span><div><span style="width:${state.documentFolders.length*10}%"></span></div></div><div class="folder-manager-list">${state.documentFolders.length?state.documentFolders.map(folder=>{const count=state.documents.filter(doc=>doc.folderId===folder.id).length;return `<div><span class="folder-tag blue">${svg('folder')}</span><span><strong>${esc(folder.name)}</strong><small>${count} document${count===1?'':'s'}</small></span><button type="button" class="mini-btn delete-folder" data-id="${folder.id}">${svg('trash')}</button></div>`}).join(''):'<div class="folder-empty">No custom folders have been created.</div>'}</div><div class="folder-manager-note">Removing a folder does not delete its documents.</div></div><div class="modal-actions"><button type="button" class="primary folder-done">Done</button></div>`;dialog.querySelector('.folder-close').onclick=()=>dialog.close();dialog.querySelector('.folder-done').onclick=()=>dialog.close();dialog.querySelector('.folder-create-form').onsubmit=async event=>{event.preventDefault();const name=String(new FormData(event.currentTarget).get('folderName')||'').trim();if(!name)return;if(state.documentFolders.length>=10){toast('GarageLog supports up to 10 custom document folders');return}if(state.documentFolders.some(folder=>folder.name.toLowerCase()===name.toLowerCase())){toast('A folder with that name already exists');return}state.documentFolders.push({id:makeRecordId('folder'),name});await saveNow();draw();render()};dialog.querySelectorAll('.delete-folder').forEach(button=>button.onclick=async()=>{const id=button.dataset.id;state.documentFolders=state.documentFolders.filter(folder=>folder.id!==id);state.documents.forEach(doc=>{if(doc.folderId===id)doc.folderId=null});await saveNow();draw();render()})};draw();dialog.showModal()}
...
function documentMatchesSearch(doc,value=documentSearchQuery){
 ...
 return [doc?.name,doc?.originalName,normalizedCategory,documentFolderById(doc?.folderId)?.name,vehicleNameFromId(doc?.vehicleId||state.activeVehicleId),doc?.ocrText,...documentTags(doc||{})].filter(Boolean).join(' ').toLowerCase().includes(query)
}
...
function dashboard(){
 ...
 <section class="dashboard-tile-row dashboard-primary-row">
   ...
   <article class="card command-tile service-history-tile">
     ...
     <div class="compact-table"><div class="compact-table-head"><span>Date</span><span>Service</span><span>Shop</span><span>Cost</span></div>${serviceHistory.length?serviceHistory.map(x=>`<div class="compact-table-row"><span title="${esc(x.date)}">${esc(shortDate(x.date))}</span><span>${esc(x.notes||x.category)}</span><span>${esc(x.vendor)}</span><strong>${money(x.amount)}</strong></div>`).join(''):'<div class="tile-empty">Service expenses will appear here.</div>'}</div>
   </article>
 ...
 <section class="dashboard-tile-row dashboard-secondary-row">
   ...
   <article class="card command-tile reminders-tile">
     ...
     <div class="reminder-quick-list">${reminders.length?reminders.map(r=>`<button onclick="goPage('Reminders')"><span class="round-icon ${maintenanceTone(r.name)}">${svg(maintenanceIcon(r.name))}</span><span><strong>${esc(r.name)}</strong><small>${esc(r.due)}</small></span><em class="${r.status==='Overdue'?'status-bad':String(r.status).toLowerCase().includes('soon')?'status-warn':'status-good'}">${esc(r.status)}</em></button>`).join(''):'<div class="tile-empty">No reminders have been added.</div>'}</div>
   </article>
 </section>`;
}
 ...
 `<div class="maintenance-tabs">${filterTabs.map(tab=>`<button class="${currentFilter===tab.key?'active':''}" onclick="setFilter('${tab.key}')"><span>${tab.label}</span><b>${tab.count}</b></button>`).join('')}</div>
   <section class="card maintenance-list-card">
     <div class="maintenance-list-head">
       <span>Maintenance Item</span><span>Interval</span><span>Status / Progress</span><span>Next Due</span><span>Action</span>
     </div>
     <div class="maintenance-record-list">
       ${items.length?items.map(m=>{
         ...
         return `<article class="maintenance-record">
     ...
     <div class="maintenance-list-footer">Showing ${items.length} of ${maintenance.length} items</div>
   </section>
   </div>
...
function expenses(){
 ...
 const filtered=allExpenses.filter(item=>currentFilter==='All'||groupCategory(item.category)===currentFilter);
       ...
       <article class="card expense-table-card">
         <div class="expense-table-scroll">
           <table class="expense-table">
             <thead><tr><th>Date</th><th>Category</th><th>Vendor</th><th>Notes</th><th>Amount</th><th>Vehicle</th><th>Attachment</th><th></th></tr></thead>
             <tbody>${displayed.length?displayed.map(item=>{const index=state.expenses.indexOf(item);const category=groupCategory(item.category);return `<tr>
               ...
               <td class="expense-notes">${esc(item.notes||'—')}</td>
         ...
         <footer class="expense-table-footer"><span>Showing ${displayed.length?1:0} to ${displayed.length} of ${filtered.length} expenses</span></footer>
       </article>
...
function documents(){
 ...
 const categories=['All Files','Receipts','Registration','Insurance','Manuals','Warranties','Photos'],customFolders=state.documentFolders.slice(0,10);
 ...
 const folderFilterId=String(currentFilter).startsWith('folder:')?String(currentFilter).slice(7):null;
 const visibleByCategory=docsAll.filter(doc=>currentFilter==='All'||currentFilter==='All Files'||doc.category===currentFilter||(folderFilterId&&doc.folderId===folderFilterId));
 const query=documentSearchQuery.trim().toLowerCase();
 const docs=visibleByCategory.filter(doc=>documentMatchesSearch(doc,query));
 ...
 const filterButtons=categories.map(category=>`<button class="document-filter-pill ${(currentFilter==='All'&&category==='All Files')||currentFilter===category?'active':''}" onclick="setFilter('${category==='All Files'?'All':category}')">${esc(category)} <span>${counts[category]||0}</span></button>`).join('')+customFolders.map(folder=>`<button class="document-filter-pill custom-folder-filter ${currentFilter===`folder:${folder.id}`?'active':''}" onclick="setFilter('folder:${folder.id}')">${svg('folder')} ${esc(folder.name)} <span>${docsAll.filter(doc=>doc.folderId===folder.id).length}</span></button>`).join('');
 return `<div class="documents-page-grid"><div class="documents-main-column">
  ...
  <section class="card documents-table-card"><div class="documents-table-scroll"><table class="documents-table"><thead><tr><th>File Name</th><th>Category</th><th>Vehicle</th><th>Date Added ↓</th><th>Tags</th><th>Size</th><th></th></tr></thead><tbody>${visibleByCategory.map(doc=>{const tags=documentTags(doc),tone=documentTone(doc.category),folder=documentFolderById(doc.folderId),matches=documentMatchesSearch(doc,query);return `<tr data-document-index="${doc._index}" ${matches?'':'hidden'}><td><button class="document-file-button" onclick="openDocument(${doc._index})"><span class="document-file-cell">${fileTypeIcon(doc)}<span><strong>${esc(doc.name)}</strong><small>${doc.storedName?`${esc(fileTypeMeta(doc).label)} · ${documentOcrStatusLabel(doc)}`:'Metadata record'}</small></span></span></button></td><td><span class="document-category-pill ${tone}">${esc(doc.category)}</span>${folder?`<small class="document-folder-name">${svg('folder')} ${esc(folder.name)}</small>`:''}</td><td>${esc(vehicleNameFromId(doc.vehicleId))}</td><td class="document-date-cell">${documentDisplayDate(doc)}</td><td><div class="document-tag-list">${tags.map(tag=>`<span class="document-tag ${tone}">${esc(tag)}</span>`).join('')}</div></td><td>${esc(doc.size||formatBytes(doc.bytes))}</td><td class="document-menu-cell">${documentRowMenu(doc._index,doc)}</td></tr>`}).join('')}<tr class="document-search-empty" ${docs.length?'hidden':''}><td colspan="7" class="empty">No documents match this category, folder, or OCR search.</td></tr></tbody></table></div><div class="documents-table-footer" data-document-total="${visibleByCategory.length}">Showing ${docs.length?1:0} to ${docs.length} of ${visibleByCategory.length} items</div></section>
  <div class="documents-bottom-grid">
   <section class="card document-mini-card"><div class="section-title"><h3>Folders & Categories</h3></div><div class="folder-category-list">${categories.filter(category=>category!=='All Files'&&counts[category]).map(category=>`<div><span class="folder-tag ${documentTone(category)}">${svg('folder')}</span><b>${esc(category)}</b><small>${counts[category]} file${counts[category]===1?'':'s'}</small></div>`).join('')}${customFolders.map(folder=>`<div><span class="folder-tag blue">${svg('folder')}</span><b>${esc(folder.name)}</b><small>${docsAll.filter(doc=>doc.folderId===folder.id).length} files</small></div>`).join('')}</div><button class="link-button folder-manage-link" onclick="openDocumentFolderManager()">Manage Folders</button></section>
   <section class="card document-mini-card document-search-card"><div class="section-title"><h3>Search Documents (OCR)</h3><div class="ocr-heading-actions"><button type="button" class="link-button" onclick="openOcrStatus()">OCR Status</button><button type="button" class="link-button document-index-button" onclick="indexAllDocuments()" ${documentIndexProgress.running?'disabled':''}>${documentIndexProgress.running?'Indexing…':'Index All'}</button></div></div><p class="muted search-copy">Search names, tags, folders, PDF text, Word documents, and text found in images.</p><div class="document-ocr-search"><span>${svg('search')}</span><input id="documentOcrSearch" type="text" value="${esc(documentSearchQuery)}" placeholder="Search document contents..." autocomplete="off" oninput="setDocumentSearchQuery(this.value)"></div>${documentIndexProgress.running?`<div class="ocr-index-progress-copy">Indexing ${documentIndexProgress.completed} of ${documentIndexProgress.total}${documentIndexProgress.currentName?` · ${esc(documentIndexProgress.currentName)}`:''}…</div><div class="ocr-index-progress"><span style="width:${documentIndexProgress.total?documentIndexProgress.completed/documentIndexProgress.total*100:0}%"></span></div>`:''}<div class="document-popular-searches"><span>Popular searches</span><div>${popularSearches.map(tag=>`<button onclick="useDocumentSearch('${tag}')">${tag}</button>`).join('')}</div></div></section>
...
function reminders(){
 ...
 const items=all.filter(x=>activeTab==='All'||statusKey(x.status)===activeTab);
...
function reports(){
 ...
 const maintenanceHtml=maintenanceBreakdown.length?`<div class="reports-donut-layout refined"><div class="reports-donut" style="background:${donutBackground}"><span><strong>${money(maintenanceSpend)}</strong><small>${maintenanceBreakdown.reduce((count,item)=>count+item.count,0)} entries</small></span></div><div class="reports-breakdown-list refined">${maintenanceBreakdown.map(item=>`<div class="reports-breakdown-row"><div class="breakdown-row-heading"><span><i style="background:${item.color}"></i>${esc(item.label)}</span><b>${money(item.value)}</b></div><div class="breakdown-progress"><span style="width:${maintenanceSpend?(item.value/maintenanceSpend)*100:0}%;background:${item.color}"></span></div><small>${item.count} entr${item.count===1?'y':'ies'} · ${money(item.average)} average · ${maintenanceSpend?((item.value/maintenanceSpend)*100).toFixed(1):'0.0'}%</small></div>`).join('')}</div></div>`:`<div class="report-chart-empty"><strong>No maintenance expenses in this period</strong><p>Maintenance, repair, and parts expenses will appear here using their entered dates.</p></div>`;
 ...
 return `<div class="reports-page-grid">
  <div class="reports-main-column">
   ...
   <div class="reports-chart-grid">
    ...
    <section class="card reports-chart-card maintenance-breakdown-card"><div class="section-title"><h3>Maintenance Breakdown</h3><strong class="chart-summary">Recorded expenses</strong></div>${maintenanceHtml}</section>
...
window.clearAllNotifications=async function(){if(!canWrite()){toast('Read-only accounts cannot clear notifications');return}const items=buildNotificationItems();if(!items.length)return;if(!confirm(`Clear ${items.length} notification${items.length===1?'':'s'} from the list? The underlying reminders and records will not be deleted.`))return;const settings=notificationSettings(),ids=new Set(items.map(item=>item.id));settings.dismissedIds=[...new Set([...(settings.dismissedIds||[]),...ids])].slice(-500);settings.readIds=(settings.readIds||[]).filter(id=>!ids.has(id));state.systemNotices=(state.systemNotices||[]).filter(notice=>!ids.has(String(notice.id)));renderNotificationCenter();try{await saveNow();toast('Notifications cleared')}catch(err){toast(err.message)}}
window.openGarageNotification=async function(id,page,recordId,url){const settings=notificationSettings();if(!settings.readIds.includes(id))settings.readIds.push(id);notificationPanelOpen=false;current=page||'Dashboard';currentFilter='All';render();if(canWrite())try{await saveNow()}catch{};if(url){window.open(url,'_blank','noopener,noreferrer');return}requestAnimationFrame(()=>{if(recordId){const target=document.querySelector(`[data-record-id="${CSS.escape(recordId)}"]`);target?.scrollIntoView({behavior:'smooth',block:'center'})}})}
window.createTestNotification=async function(){if(!canWrite()){toast('Read-only accounts cannot create a test notification');return}state.systemNotices=Array.isArray(state.systemNotices)?state.systemNotices:[];state.systemNotices.push({id:`system:test:${Date.now()}`,title:'Bell notifications are working',detail:'This test notice was created locally from the notification center.',page:'Dashboard',tone:'blue',icon:'bell',createdAt:new Date().toISOString()});notificationPanelOpen=true;try{await saveNow();render();toast('Test notification added')}catch(err){toast(err.message)}}
...
function render(){
 ...
 const page=({Dashboard:dashboard,Garage:garage,Maintenance:maintenance,Expenses:expenses,Documents:documents,Reminders:reminders,Reports:reports,Profile:profileSettings})[current]||dashboard;content.innerHTML=(current==='Profile'?'':permissionNotice())+updateAvailableBanner()+page();updateProfileChrome();applySearch();applyPermissionUi();if(current==='Documents')requestAnimationFrame(()=>requestAnimationFrame(syncDocumentsListHeight));if(current==='Expenses')requestAnimationFrame(()=>requestAnimationFrame(syncExpensesListHeight));if(current==='Reminders'&&reminderViewMode==='calendar')requestAnimationFrame(()=>requestAnimationFrame(syncReminderCalendarHeight));if(current==='Profile')requestAnimationFrame(()=>{const form=document.querySelector('.managed-user-form');if(form)syncManagedUserAccessFields(form)});renderNotificationCenter();
}
window.goPage=p=>{current=p;currentFilter='All';if(p==='Reports')reportViewMode='dashboard';render()}
window.openVehicleRecords=async function(vehicleId,page){closeRecordModal();activateVehicle(String(vehicleId),false);current=page;currentFilter='All';if(canWrite())await saveNow();render()}
...
const blankVehicle={year:'',make:'',model:'',trim:'',type:'Car',powertrain:'Gasoline / Internal Combustion',vin:'',engine:'',transmission:'',drivetrain:'',color:'',mileage:0};
...
const configs={
 ...
 document:{title:isEdit?'Edit Document':'Upload Document',subtitle:isEdit?'Rename and organize the stored document metadata.':`Store a file for ${vehicleFullName()}.`,fields:isEdit?[['File Name','text',item?.name||'','full'],['Category','select',normalizeDocumentCategory(item?.category||'Receipts'),'half',['Receipts','Registration','Insurance','Manuals','Warranties','Photos','Other']],['Custom Folder','select',documentFolderById(item?.folderId)?.name||'No custom folder','half',['No custom folder',...state.documentFolders.map(folder=>folder.name)]],['Tags','text',(item?.tags||documentTags(item)).join(', '),'full'],['Expiration Date','date',item?.expiresOn||'','half']]:[['File','file','','full'],['Category','select','Receipts','half',['Receipts','Registration','Insurance','Manuals','Warranties','Photos','Other']],['Custom Folder','select','No custom folder','half',['No custom folder',...state.documentFolders.map(folder=>folder.name)]],['Display Name','text','','half'],['Tags','text','','half'],['Expiration Date','date','','half']]}
};return configs[type]}
function expenseFieldsHtml(item={}){
 const category=item?.category||'Maintenance',isFuel=category==='Fuel';
 return `<div class="expense-entry-workspace"><section class="expense-entry-card"><div class="expense-entry-section-heading"><h5>Transaction details</h5></div><div class="expense-entry-grid"><label class="expense-entry-field expense-date-field"><span>Expense Date *</span><input name="Date" type="date" value="${esc(item?.date||new Date().toISOString().slice(0,10))}" required></label><label class="expense-entry-field"><span>Category *</span><select name="Category" required>${['Maintenance','Repair','Fuel','Parts','Insurance','Registration','Other'].map(option=>`<option ${option===category?'selected':''}>${option}</option>`).join('')}</select></label><label class="expense-entry-field"><span>Vendor</span><input name="Vendor" type="text" value="${esc(item?.vendor||'')}" placeholder="Shop, station, insurer, or seller"></label><label class="expense-entry-field expense-amount-field"><span>Amount *</span><div class="expense-currency-input"><span aria-hidden="true">$</span><input id="expenseAmountInput" name="Amount" type="number" value="${esc(item?.amount??'')}" min="0" step="0.01" inputmode="decimal" placeholder="0.00" aria-label="Expense amount in US dollars" required></div></label><label class="expense-entry-field expense-notes-field"><span>Notes</span><textarea name="Notes" rows="2" placeholder="What was purchased or serviced?">${esc(item?.notes||'')}</textarea></label></div></section><section id="expenseFuelFields" class="expense-fuel-fields" ${isFuel?'':'hidden'}><div class="expense-fuel-heading"><span>${svg('fuel')}</span><strong>Fuel economy details</strong></div><div class="expense-fuel-grid"><label>Gallons<input name="Gallons" type="number" value="${esc(item?.gallons??'')}" min="0" step="0.001" placeholder="e.g. 17.245"></label><label>Odometer<input name="Odometer" type="number" value="${esc(item?.odometer??'')}" min="0" step="1" placeholder="Mileage at fill-up"></label><label>MPG override<input name="MPG" type="number" value="${esc(item?.mpg??'')}" min="0" step="0.1" placeholder="Optional"></label><label class="expense-full-tank"><input name="Full Tank" type="checkbox" ${item?.fullTank===false?'':'checked'}><span><strong>Full tank fill-up</strong></span></label></div></section></div>`
}
function syncExpenseFuelFields(){const category=document.querySelector('#modalFields [name="Category"]'),panel=document.getElementById('expenseFuelFields');if(!category||!panel)return;panel.hidden=category.value!=='Fuel'}
...
window.openModal=function(type,index=null){
 ...
 editing={type,index};const c=modalConfig(type,index),modal=document.getElementById('modal');document.getElementById('modalTitle').textContent=c.title;document.getElementById('modalSubtitle').textContent=c.subtitle;
 modal.classList.toggle('vehicle-add-dialog',type==='vehicle-add');modal.classList.toggle('vehicle-edit-dialog',type==='vehicle');modal.classList.toggle('document-upload-dialog',type==='document'&&index===null);modal.classList.toggle('expense-dialog',type==='expense');
 clearDocumentUploadPreview();
 const saveModal=document.getElementById('saveModal');if(saveModal)saveModal.innerHTML=type==='expense'?`${svg('dollar')} ${index!==null&&index!==undefined?'Save Changes':'Add Expense'}`:'Save';
 const modalFields=document.getElementById('modalFields');
 if(type==='vehicle'){
   const vehicle=state.vehicles[index];modalFields.innerHTML=vehicleEditFieldsHtml(vehicle,index);modalFields.querySelector('.vehicle-delete-from-edit').onclick=()=>openVehicleDeletePrompt(index);const archiveButton=modalFields.querySelector('.vehicle-archive-from-edit'),restoreButton=modalFields.querySelector('.vehicle-restore-from-edit');if(archiveButton)archiveButton.onclick=()=>openVehicleArchivePrompt(index);if(restoreButton)restoreButton.onclick=()=>openVehicleRestorePrompt(index)
 }else{
   modalFields.innerHTML=type==='document'&&index===null?documentUploadFieldsHtml():type==='expense'?expenseFieldsHtml(index!==null&&index!==undefined?state.expenses[index]:{}):c.fields.map(([n,t,v,w,options,accept])=>{const cls=w==='full'?'full':'';if(t==='select')return `<label class="${cls}">${n}<select name="${n}" required>${options.map(o=>`<option ${String(o)===String(v)?'selected':''}>${esc(o)}</option>`).join('')}</select></label>`;const required=t==='file'&&type==='document'&&index===null?'required':'';const acceptAttr=accept?`accept="${accept}"`:'';return `<label class="${cls}">${n}<input name="${n}" type="${t}" value="${t==='file'?'':esc(v)}" ${required} ${acceptAttr} step="${t==='number'?'0.01':'any'}"></label>`}).join('')
 }
 modal.showModal();
 if(type==='document'&&index===null)initializeDocumentUploadDialog();if(type==='expense')initializeExpenseDialog()
}
...
function openInfoModal(kind){
 ...
 if(kind==='profile'){
   ...
   body.innerHTML=`<div class="about-logo">${svg('shield')}<div><strong>GarageLog 0.7.2</strong><small>Local-first self-hosted release</small></div></div><div class="info-section"><h4>About</h4><p>GarageLog keeps vehicle, maintenance, expense, reminder, and document records on your own instance.</p></div><div class="info-section"><h4>Help</h4><p>Use Garage for vehicle details, Maintenance for service intervals, Documents for local files, and Reminders for date- or mileage-based rules.</p></div><div class="info-callout">GarageLog authentication is local to this self-hosted instance. Account records and vehicle data remain in the GarageLog data folder.</div>`;
 }else{
...
window.printServiceHistory=function(){
 ...
 const serviceRows=services.map(x=>`<tr><td>${esc(x.date)}</td><td>${esc(x.notes||x.category)}</td><td>${esc(x.vendor||'—')}</td><td>${esc(x.category)}</td><td class="amount">${money(x.amount)}</td></tr>`).join('')||'<tr><td colspan="5">No completed service expenses recorded.</td></tr>';
...
window.printExpenseReport=function(mode='full'){
 ...
 const rows=expenses.map(item=>`<tr><td>${esc(item.date||'—')}</td><td>${esc(item.category||'Other')}</td><td>${esc(item.vendor||'—')}</td><td>${esc(item.notes||'—')}</td><td>${esc(attachedName(item))}</td><td class="amount">${money(item.amount)}</td></tr>`).join('')||'<tr><td colspan="6">No expenses were recorded for this period.</td></tr>';
...
document.getElementById('modalForm').addEventListener('submit',async e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const {type,index}=editing;const fd=new FormData(e.target);try{
 if(type==='mileage'){state.mileage=Number(fd.get('Mileage'));state.mileageHistory.push({date:new Date().toISOString(),mileage:state.mileage,source:fd.get('Source')});persistActiveVehicle()}
   ...
   const now=new Date().toISOString(),x={...existing,id:existing?.id||makeRecordId('expense'),vehicleId:existing?.vehicleId||state.activeVehicleId,date:expenseDate,category,vendor:fd.get('Vendor'),notes:fd.get('Notes'),amount:Number(fd.get('Amount')),createdAt:existing?.createdAt||now,updatedAt:now};
   if(category==='Fuel'){x.gallons=finitePositive(fd.get('Gallons'));x.odometer=finitePositive(fd.get('Odometer'));x.mpg=finitePositive(fd.get('MPG'));x.fullTank=fd.get('Full Tank')==='on'}else{delete x.gallons;delete x.odometer;delete x.mpg;delete x.fullTank}
 ...
 else if(type==='document'){const folderName=String(fd.get('Custom Folder')||'No custom folder'),folder=state.documentFolders.find(entry=>entry.name===folderName),tags=String(fd.get('Tags')||'').split(',').map(tag=>tag.trim().toLowerCase()).filter(Boolean).slice(0,12),expiresOn=String(fd.get('Expiration Date')||'');if(index!==null){const doc=state.documents[index];doc.name=String(fd.get('File Name')||doc.name).trim()||doc.name;doc.category=normalizeDocumentCategory(fd.get('Category'));doc.folderId=folder?.id||null;doc.tags=tags;doc.expiresOn=expiresOn||null;doc.updatedAt=new Date().toISOString()}else{const file=fd.get('File');if(!(file instanceof File)||!file.size)throw new Error('Choose a file to upload.');const createExpense=fd.get('Create Expense')==='on',expenseAmount=Number(fd.get('Expense Amount')||0);if(createExpense&&(!(expenseAmount>0)))throw new Error('Enter an expense amount greater than zero.');const upload=new FormData();upload.append('file',file);const r=await fetch('/api/documents',{method:'POST',body:upload});if(!r.ok)throw new Error((await r.json().catch(()=>({}))).error||'Upload failed');const result=await r.json();const documentId=makeRecordId('document'),expenseId=createExpense?makeRecordId('expense'):null;const documentRecord={id:documentId,vehicleId:state.activeVehicleId,name:String(fd.get('Display Name')||'').trim()||file.name,originalName:result.originalName||file.name,extension:result.extension||fileExtension(file.name),contentType:result.contentType||file.type,category:normalizeDocumentCategory(fd.get('Category')),folderId:folder?.id||null,tags,expiresOn:expiresOn||null,date:new Date().toLocaleDateString('en-US'),addedAt:new Date().toISOString(),size:formatBytes(file.size),bytes:file.size,storedName:result.storedName,lastViewedAt:null,ocrStatus:'not-indexed',ocrText:'',linkedExpenseId:expenseId};state.documents.unshift(documentRecord);if(createExpense){const expenseDate=String(fd.get('Expense Date')||new Date().toISOString().slice(0,10)),expenseTimestamp=new Date().toISOString();state.expenses.unshift({id:expenseId,vehicleId:state.activeVehicleId,date:expenseDate,category:String(fd.get('Expense Category')||'Other'),vendor:String(fd.get('Expense Vendor')||'').trim(),notes:String(fd.get('Expense Notes')||documentRecord.name).trim()||documentRecord.name,amount:expenseAmount,linkedDocumentId:documentId,linkedDocumentStoredName:documentRecord.storedName,source:'document-upload',createdAt:expenseTimestamp,updatedAt:expenseTimestamp})}await saveNow();await indexDocumentRecord(0,{quiet:true})}}
 await saveNow();clearDocumentUploadPreview();const recordModal=document.getElementById('modal');recordModal.close();recordModal.classList.remove('document-upload-dialog');const successMessage=pendingVehicleLifecycleToast||(type==='vehicle-add'?'Vehicle added':index!==null?'Record updated':'Record added');pendingVehicleLifecycleToast=null;toast(successMessage);render();
 }catch(err){pendingVehicleLifecycleToast=null;console.error(err);toast(err.message||'Unable to save')}})
...
function applySearch(){const q=document.getElementById('globalSearch').value.trim().toLowerCase();document.querySelectorAll('.data-table tbody tr,.maintenance-preview-row,.side-row').forEach(r=>r.style.display=!q||r.textContent.toLowerCase().includes(q)?'':'none')}
...
document.querySelectorAll('[data-profile-action]').forEach(button=>button.addEventListener('click',()=>button.dataset.profileAction==='profile'?openProfilePage():openInfoModal(button.dataset.profileAction)));
...
document.addEventListener('click',event=>{const wrap=document.querySelector('.profile-wrap');if(wrap&&!wrap.contains(event.target))setProfileMenu(false);const notificationWrap=document.querySelector('.notification-wrap');if(notificationWrap&&!notificationWrap.contains(event.target)&&notificationPanelOpen)closeNotificationPanel();const activeMenu=event.target.closest('details.expense-row-menu,details.reminder-more,details.maintenance-row-menu');if(activeMenu){closeActionMenus(activeMenu)}else{closeActionMenus()}});
...
window.exportExpenses=function(){const rows=[['Date','Category','Vendor','Notes','Amount'],...activeExpenses().map(x=>[x.date,x.category,x.vendor,x.notes,x.amount])];const csv=rows.map(r=>r.map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download='garagelog-expenses.csv';a.click();URL.revokeObjectURL(a.href)}
