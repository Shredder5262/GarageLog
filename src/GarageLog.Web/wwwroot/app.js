...
let pendingExpenseDocumentContext=null;
...
function documentRelevantDateLabel(value){
  return ({Receipts:'Receipt Date',Registration:'Registration Date',Insurance:'Effective Date',Warranties:'Start Date',Photos:'Photo Date'})[normalizeDocumentCategory(value)]||'Relevant Date'
}
function documentExpenseLinkOptions(doc,index){
  const linked=linkedExpenseForDocument(doc),vehicleId=String(doc?.vehicleId||state.activeVehicleId),options=['<option value="">No linked expense</option>'];
  state.expenses.forEach(expense=>{
    if(String(expense?.vehicleId||'')!==vehicleId&&String(expense?.id||'')!==String(linked?.id||''))return;
    const attached=linkedDocumentForExpense(expense);if(attached&&attached.index!==index&&String(expense?.id||'')!==String(linked?.id||''))return;
    const selected=String(expense?.id||'')===String(linked?.id||'')?' selected':'';
    const label=[expense?.date,expense?.category,expense?.vendor||expense?.notes,money(expense?.amount)].filter(Boolean).join(' · ');
    options.push(`<option value="${esc(expense.id)}"${selected}>${esc(label)}</option>`)
  });
  return options.join('')
}
function documentExpenseLinkMarkup(doc,index){
  if(!Number.isInteger(index))return'';
  const linked=linkedExpenseForDocument(doc),linkedIndex=linked?state.expenses.indexOf(linked):-1;
  return `<section class="document-linked-expense document-linked-expense-edit full" data-document-context="Receipts,Registration,Insurance,Warranties,Other"><div class="document-expense-link-head"><div><span class="wizard-eyebrow">EXPENSE</span><p>${linked?'Linked to an expense. Change the selection to relink or choose No linked expense to unlink.':'No expense is linked to this document yet.'}</p></div></div><div class="document-expense-link-controls"><label>Linked Expense<select name="Linked Expense">${documentExpenseLinkOptions(doc,index)}</select></label>${linkedIndex>=0?`<button type="button" class="secondary" onclick="editLinkedExpenseFromDocument(${index})">${svg('edit')} Edit Expense</button>`:`<button type="button" class="secondary" onclick="createExpenseFromDocument(${index})">${svg('dollar')} Create Expense</button>`}</div></section>`
}
function documentEditFieldsHtml(doc={},index=null){
  const category=normalizeDocumentCategory(doc.category||'Receipts'),services=serviceValuesText(documentServiceValues(doc)),shop=documentShopValue(doc),linkedExpense=linkedExpenseForDocument(doc),relevantDate=doc.relevantDate||doc.startsOn||linkedExpense?.date||'';
  return `<label class="full">File Name<input name="File Name" type="text" value="${esc(doc.name||'')}" required></label><label class="full">Category<select name="Category" required>${documentCategoryOptions(category)}</select></label><label><span class="document-relevant-date-label">${esc(documentRelevantDateLabel(category))}</span><input name="Relevant Date" type="date" value="${esc(String(relevantDate).slice(0,10))}"><small class="wizard-field-help">Date this document represents. Used on the vehicle timeline; Date Added remains unchanged.</small></label><label class="document-context-field" data-document-context="Registration,Insurance,Warranties">Expiration Date<input name="Expiration Date" type="date" value="${esc(doc.expiresOn||'')}"></label><label class="full document-context-field" data-document-context="Receipts">Service<input name="Service" type="text" value="${esc(services)}" placeholder="Oil Change, Tire Rotation, Air Filter"><small class="wizard-field-help">Separate multiple services with commas.</small></label><label class="full document-context-field" data-document-context="Receipts">Shop<input name="Shop" type="text" value="${esc(shop)}" placeholder="Company or shop that performed the service"></label><label class="full">Tags<input name="Tags" type="text" value="${esc((doc.tags||documentTags(doc)).join(', '))}" placeholder="oil, receipt, 2026"></label>${documentExpenseLinkMarkup(doc,index)}`
}
...
function syncDocumentContextFields(){
  ...
  document.querySelectorAll('#modalFields .document-relevant-date-label').forEach(label=>label.textContent=documentRelevantDateLabel(category));
...
function documentVehicleDisplay(id){
  const vehicle=state.vehicles.find(v=>v.id===id)||state.vehicle,base=String(vehicle?.name||[vehicle?.year,vehicle?.make,vehicle?.model].filter(Boolean).join(' ')).trim(),trim=String(vehicle?.trim||'').trim(),showTrim=Boolean(trim)&&!base.toLowerCase().endsWith(trim.toLowerCase());
  return `<span class="document-vehicle-cell"><span>${esc(base||'Vehicle')}</span>${showTrim?`<small>${esc(trim)}</small>`:''}</span>`
}
...
function documentCoverageDates(doc){const linked=linkedExpenseForDocument(doc),start=parseDocumentDate(doc?.relevantDate||doc?.startsOn||linked?.date),expiry=parseDocumentDate(doc?.expiresOn);return{start,expiry,sortDate:expiry||start||null}}
function documentCoverageDateDisplay(doc){const {start,expiry}=documentCoverageDates(doc);if(!start&&!expiry)return '<span class="document-coverage-empty">—</span>';const format=date=>date.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}),label=documentRelevantDateLabel(doc?.category);return `<span class="document-coverage-date">${start?`<span><small>${esc(label)}</small><strong>${format(start)}</strong></span>`:''}${expiry?`<span><small>Expires</small><strong>${format(expiry)}</strong></span>`:''}</span>`}
...
function estimateDocumentExpiry(doc){
  ...
  const start=parseDocumentDate(doc?.relevantDate||doc?.startsOn||doc?.date||doc?.addedAt);if(!start)return null;
...
function mobileReceiptReviewMarkup(doc,index){
 ...
 const ocr=doc.receiptOcr||{},vehicle=state.vehicles.find(item=>String(item?.id||'')===String(doc.vehicleId||'')),currentOdometer=finitePositive(vehicle?.mileage),suggestedOdometer=finitePositive(ocr.odometer)||currentOdometer,captureType=String(doc.mobileCaptureType||ocr.captureType||'receipt').toLowerCase(),processing=String(doc.ocrStatus||'').toLowerCase()==='indexing'||String(ocr.status||'').toLowerCase()==='processing',captured=parseDocumentDate(doc.capturedUtc||doc.addedAt),date=String(ocr.purchaseDate||captured?.toISOString().slice(0,10)||new Date().toISOString().slice(0,10)),merchant=String(ocr.merchant||doc.shop||''),amount=processing?'':(Number(ocr.amount)>0?ocr.amount:''),gallons=processing?'':(Number(ocr.gallons)>0?ocr.gallons:''),odometer=processing?'':(suggestedOdometer??''),price=processing?null:ocr.pricePerGallon,disabled=processing?' disabled':'',kind=captureType==='pump'?'PUMP DISPLAY':'PAPER RECEIPT',missingPumpValues=!processing&&captureType==='pump'&&(!amount||!gallons),fullTankChecked=ocr.fullTank===false?'':' checked';
...
window.approveMobileReceipt=async function(index){
 ...
 if(odometer!==null&&(!Number.isFinite(odometer)||odometer<=0||odometer>10000000)){toast('Enter a valid odometer reading or leave it blank');return}
 if(fullTank&&gallons!==null&&odometer===null){toast('Enter the odometer reading, or uncheck Full-tank fill-up');return}
 const previous=JSON.parse(JSON.stringify(doc)),vehicleIndex=state.vehicles.findIndex(item=>String(item?.id||'')===String(doc.vehicleId||'')),previousVehicle=vehicleIndex>=0?JSON.parse(JSON.stringify(state.vehicles[vehicleIndex])):null,expenseId=makeRecordId('expense'),now=new Date().toISOString(),expense={id:expenseId,vehicleId:doc.vehicleId,date,category:'Fuel',vendor:merchant,notes:'Fuel receipt captured with GarageLog Mobile',amount,fullTank,linkedDocumentId:doc.id,linkedDocumentStoredName:doc.storedName,source:'mobile-receipt',createdAt:now,updatedAt:now};
 if(gallons!==null)expense.gallons=gallons;
 if(odometer!==null)expense.odometer=odometer;
 if(odometer!==null&&vehicleIndex>=0){
  const vehicle=state.vehicles[vehicleIndex];vehicle.mileageHistory=(vehicle.mileageHistory||[]).filter(reading=>String(reading?.expenseId||'')!==String(expenseId));vehicle.mileageHistory.push({date,mileage:odometer,source:'Fuel entry',expenseId});vehicle.mileageHistory.sort((a,b)=>(parseRecordDate(a.date)?.getTime()||0)-(parseRecordDate(b.date)?.getTime()||0));if(odometer>Number(vehicle.mileage||0))vehicle.mileage=odometer;
 ...
 state.expenses.unshift(expense);doc.linkedExpenseId=expenseId;doc.reviewStatus='Approved';doc.reviewedAt=now;doc.shop=merchant;doc.receiptOcr={...(doc.receiptOcr||{}),merchant,amount,gallons,odometer,fullTank,purchaseDate:date,reviewedAt:now};
 try{await saveNow();dialog?.close();render();toast('Receipt approved and fuel expense created')}catch(error){state.expenses=state.expenses.filter(item=>item.id!==expenseId);state.documents[index]=previous;if(vehicleIndex>=0&&previousVehicle){state.vehicles[vehicleIndex]=previousVehicle;if(String(previousVehicle.id)===String(state.activeVehicleId)){state.vehicle=previousVehicle;state.mileage=Number(previousVehicle.mileage||0);state.mileageHistory=previousVehicle.mileageHistory||[]}}console.error(error);toast(error.message||'Unable to approve the receipt')}
};
...
function documentMatchesSearch(doc,value=documentSearchQuery){
 ...
 const normalizedCategory=normalizeDocumentCategory(doc?.category);
  return searchMatches([doc?.name,doc?.originalName,normalizedCategory,doc?.relevantDate,documentShopValue(doc),...documentServiceValues(doc),vehicleNameFromId(doc?.vehicleId||state.activeVehicleId),doc?.ocrText,...documentTags(doc||{})].filter(Boolean).join(' '),query)
}
...
function printRecallCampaigns(items,label){
 ...
 const vehicle=recallModalVehicle||activeVehicle(),popup=openCenteredWindow('','garageLogRecallPrint',980,820);if(!popup){toast('Allow pop-ups to print recall information');return}
...
function timelineVehicleGraphic(vehicle){
 ...
 return `<img class="timeline-vehicle-${String(type||'Car').toLowerCase()}" src="${images[type]||images.Car}" alt="" aria-hidden="true" draggable="false">`
}
...
function printGenericReportTemplate(id){const template=reportTemplateById(id),range=reportRangeForCurrentVehicle(),expenses=expensesWithinRange(activeExpenses(),range),maintenance=activeMaintenance(),documents=activeDocuments(),reminders=activeReminders(),vehicle=activeVehicle();let body='';
 ...
 else if(id==='fuel-efficiency'){const fuel=expenses.filter(item=>String(item.category)==='Fuel'),points=fuelEconomyPoints().filter(point=>point.date>=range.start&&point.date<=range.end),total=fuel.reduce((sum,item)=>sum+Number(item.amount||0),0),avg=points.length?points.reduce((sum,item)=>sum+item.value,0)/points.length:null,mpgByExpenseId=new Map(points.map(point=>[String(point.item?.id||''),point.value]));const rows=fuel.map(item=>{const displayMpg=fuelMpgFromRecord(item)||mpgByExpenseId.get(String(item.id||''));return `<tr><td>${esc(item.date)}</td><td>${esc(item.vendor||'—')}</td><td>${item.gallons?`${Number(item.gallons).toFixed(2)} gal`:'—'}</td><td>${item.odometer?`${number(item.odometer)} mi`:'—'}</td><td>${displayMpg?`${Number(displayMpg).toFixed(1)} MPG`:'—'}</td><td class="amount">${money(item.amount)}</td></tr>`}).join('');body=`<div class="metric-grid"><div class="metric"><span>Fuel Spend</span><strong>${money(total)}</strong></div><div class="metric"><span>Fuel Entries</span><strong>${fuel.length}</strong></div><div class="metric"><span>Average MPG</span><strong>${avg?avg.toFixed(1):'—'}</strong></div></div><h2>Fuel Entries</h2>${rows?`<table><thead><tr><th>Date</th><th>Vendor</th><th>Gallons</th><th>Odometer</th><th>MPG</th><th class="amount">Amount</th></tr></thead><tbody>${rows}</tbody></table>`:'<div class="empty">No fuel entries exist in this period.</div>'}`}
 ...
 else if(id==='registration-insurance'){const matches=[...expenses.map(item=>({...item,kind:'Expense',title:item.category,detail:item.vendor||item.notes,date:item.date})),...documents.map(item=>({...item,kind:'Document',title:item.name,detail:item.category,date:item.relevantDate||item.startsOn||item.addedAt||item.date})),...reminders.map(item=>({...item,kind:'Reminder',title:item.name,detail:item.rule,date:item.due}))].filter(item=>/registration|insurance/i.test(`${item.title||''} ${item.detail||''}`));const rows=matches.map(item=>`<tr><td>${esc(item.kind)}</td><td>${esc(item.title||'—')}</td><td>${esc(item.detail||'—')}</td><td>${esc(item.date||'—')}</td><td>${item.amount?money(item.amount):esc(item.status||'—')}</td></tr>`).join('');body=`<h2>Registration & Insurance Records</h2>${rows?`<table><thead><tr><th>Type</th><th>Record</th><th>Details</th><th>Date / Due</th><th>Amount / Status</th></tr></thead><tbody>${rows}</tbody></table>`:'<div class="empty">No registration or insurance records were found.</div>'}`}
...
window.openModal=function(type,index=null){
 ...
 if(type==='vehicle-add'){
  ...
  if(type==='document')modalFields.innerHTML=index===null||index===undefined?documentUploadFieldsHtml():documentEditFieldsHtml(state.documents[index],index);
...
window.createExpenseFromDocument=function(index){
 if(!canWrite()){toast('This account has read-only access.');return}
 const doc=state.documents[index];if(!doc){toast('Document not found');return}
 const linked=linkedExpenseForDocument(doc);if(linked){const linkedIndex=state.expenses.indexOf(linked);if(linkedIndex>=0){editLinkedExpenseFromDocument(index);return}}
 const form=document.getElementById('modalForm'),field=name=>form?.elements?.namedItem(name)?.value;
 pendingExpenseDocumentContext={index,prefill:{date:String(field('Relevant Date')||doc.relevantDate||doc.startsOn||doc.receiptOcr?.purchaseDate||new Date().toISOString().slice(0,10)).slice(0,10),vendor:String(field('Shop')||documentShopValue(doc)||''),notes:String(field('Service')||documentServiceValues(doc).join(', ')||doc.name||''),category:documentExpenseCategorySuggestion(field('Category')||doc.category),amount:Number(doc.receiptOcr?.amount||0)>0?Number(doc.receiptOcr.amount):null,coverage:normalizeDocumentCategory(field('Category')||doc.category)==='Warranties'?'Warranty':'None'}};
 const modal=document.getElementById('modal');if(modal?.open)modal.close();modal?.classList.remove('document-upload-dialog','expense-dialog');editing=null;openModal('expense');
 const prefill=pendingExpenseDocumentContext?.prefill||{},date=document.querySelector('#modalFields [name="Date"]'),category=document.querySelector('#modalFields [name="Category"]'),vendor=document.querySelector('#modalFields [name="Vendor"]'),notes=document.querySelector('#modalFields [name="Notes"]'),amount=document.querySelector('#modalFields [name="Amount"]'),coverage=document.querySelector('#modalFields [name="Coverage"]');
 if(date)date.value=prefill.date||date.value;if(category)category.value=prefill.category||category.value;if(vendor)vendor.value=prefill.vendor||'';if(notes)notes.value=prefill.notes||'';if(amount&&prefill.amount!==null)amount.value=Number(prefill.amount).toFixed(2);if(coverage)coverage.value=prefill.coverage||'None';syncExpenseFuelFields();syncExpenseCoverageFields();document.getElementById('modalTitle').textContent='Create Expense from Document';document.getElementById('modalSubtitle').textContent=`The new expense will be linked to ${doc.name||'this document'}.`
}
window.editLinkedExpenseFromDocument=function(index){
 const doc=state.documents[index],linked=linkedExpenseForDocument(doc);if(!linked){toast('No linked expense found');return}const expenseIndex=state.expenses.indexOf(linked);if(expenseIndex<0){toast('Linked expense not found');return}
 pendingExpenseDocumentContext=null;const modal=document.getElementById('modal');if(modal?.open)modal.close();modal?.classList.remove('document-upload-dialog','expense-dialog');editing=null;openModal('expense',expenseIndex)
}
...
function closeRecordModal(){const modal=document.getElementById('modal');clearDocumentUploadPreview();if(modal?.open)modal.close();modal?.classList.remove('document-upload-dialog','expense-dialog');editing=null;pendingExpenseDocumentContext=null;const form=document.getElementById('modalForm');if(form)form.reset()}
...
document.getElementById('modalForm').addEventListener('submit',async e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const form=e.currentTarget;if(form.dataset.submitting==='true')return;const {type,index}=editing,fd=new FormData(form),submitter=e.submitter||form.querySelector('button[type="submit"]:not([value="cancel"]),input[type="submit"]:not([value="cancel"])'),submitControls=[...form.querySelectorAll('button[type="submit"],input[type="submit"]')].map(control=>({control,disabled:control.disabled})),originalSubmitHtml=submitter instanceof HTMLButtonElement?submitter.innerHTML:null,originalSubmitValue=submitter instanceof HTMLInputElement?submitter.value:null;let skipFinalSave=false;form.dataset.submitting='true';submitControls.forEach(({control})=>control.disabled=true);if(type==='document'&&(index===null||index===undefined)&&submitter){if(submitter instanceof HTMLButtonElement)submitter.innerHTML=`${svg('upload')} Uploading…`;else if(submitter instanceof HTMLInputElement)submitter.value='Uploading…'}try{
 if(type==='mileage'){state.mileage=Number(fd.get('Mileage'));state.mileageHistory.push({date:new Date().toISOString(),mileage:state.mileage,source:fd.get('Source')});persistActiveVehicle()}
   ...
   const sourceDocument=!existing&&Number.isInteger(pendingExpenseDocumentContext?.index)?state.documents[pendingExpenseDocumentContext.index]:null,now=new Date().toISOString(),x={...existing,id:existing?.id||makeRecordId('expense'),vehicleId:existing?.vehicleId||sourceDocument?.vehicleId||state.activeVehicleId,date:expenseDate,category,vendor:fd.get('Vendor'),notes:fd.get('Notes'),amount,coverageType,coveredAmount:coverageType==='None'?null:coveredAmount,createdAt:existing?.createdAt||now,updatedAt:now};
   ...
   if(String(vehicle.id)===String(state.activeVehicleId)){state.vehicle=vehicle;state.mileage=Number(vehicle.mileage||0);state.mileageHistory=vehicle.mileageHistory}
   if(index!==null)state.expenses[index]=x;else{if(sourceDocument){x.linkedDocumentId=sourceDocument.id;x.linkedDocumentStoredName=sourceDocument.storedName;x.source='document-link';sourceDocument.linkedExpenseId=x.id;sourceDocument.relevantDate=sourceDocument.relevantDate||expenseDate;sourceDocument.updatedAt=now;if(normalizeDocumentCategory(sourceDocument.category)==='Receipts'){sourceDocument.shop=sourceDocument.shop||String(x.vendor||'');if(!Array.isArray(sourceDocument.services)||!sourceDocument.services.length)sourceDocument.services=normalizeServiceValues(x.notes||'')}}state.expenses.unshift(x)}pendingExpenseDocumentContext=null
 }
    ...
    const isEdit=index!==null&&index!==undefined,category=normalizeDocumentCategory(fd.get('Category')),tags=String(fd.get('Tags')||'').split(',').map(tag=>tag.trim().toLowerCase()).filter(Boolean).slice(0,12),services=category==='Receipts'?normalizeServiceValues(fd.get('Service')):[],shop=category==='Receipts'?String(fd.get('Shop')||'').trim():'',relevantDate=String(fd.get('Relevant Date')||'').trim(),expiresOn=['Registration','Insurance','Warranties'].includes(category)?String(fd.get('Expiration Date')||''):'';
    if(relevantDate&&expiresOn&&relevantDate>expiresOn)throw new Error(`${documentRelevantDateLabel(category)} must be on or before the expiration date.`);
    if(isEdit){
      const doc=state.documents[index],now=new Date().toISOString(),requestedLinkedExpenseId=String(fd.get('Linked Expense')||'').trim(),requestedLinkedExpense=requestedLinkedExpenseId?state.expenses.find(expense=>String(expense?.id||'')===requestedLinkedExpenseId):null,previousLinkedExpense=linkedExpenseForDocument(doc);
      if(requestedLinkedExpenseId&&!requestedLinkedExpense)throw new Error('The selected expense could not be found.');
      const requestedAttachment=requestedLinkedExpense?linkedDocumentForExpense(requestedLinkedExpense):null;if(requestedAttachment&&requestedAttachment.index!==index)throw new Error('That expense is already linked to another document.');
      doc.name=String(fd.get('File Name')||doc.name).trim()||doc.name;doc.category=category;doc.tags=tags;doc.services=services;doc.shop=shop;doc.relevantDate=relevantDate||null;doc.startsOn=null;doc.expiresOn=expiresOn||null;doc.updatedAt=now;
      if(previousLinkedExpense&&String(previousLinkedExpense.id)!==requestedLinkedExpenseId){delete previousLinkedExpense.linkedDocumentId;delete previousLinkedExpense.linkedDocumentStoredName;previousLinkedExpense.updatedAt=now}
      if(requestedLinkedExpense){requestedLinkedExpense.linkedDocumentId=doc.id;requestedLinkedExpense.linkedDocumentStoredName=doc.storedName;requestedLinkedExpense.updatedAt=now;doc.linkedExpenseId=requestedLinkedExpense.id}else doc.linkedExpenseId=null;
      const linkedExpense=linkedExpenseForDocument(doc);if(linkedExpense&&category==='Receipts'){if(services.length){linkedExpense.services=services;linkedExpense.notes=services.join(', ')||linkedExpense.notes||doc.name}if(shop)linkedExpense.vendor=shop;linkedExpense.updatedAt=now}
    }else{
      ...
      const documentId=makeRecordId('document'),expenseId=createExpense?makeRecordId('expense'):null,now=new Date().toISOString(),expenseDate=createExpense?String(fd.get('Expense Date')||new Date().toISOString().slice(0,10)):'';
      const documentRecord={id:documentId,vehicleId:state.activeVehicleId,name:String(fd.get('Display Name')||'').trim()||file.name,originalName:result.originalName||file.name,extension:result.extension||fileExtension(file.name),contentType:result.contentType||file.type,category,tags,services,shop,relevantDate:relevantDate||(expenseDate||null),expiresOn:expiresOn||null,date:new Date().toLocaleDateString('en-US'),addedAt:now,size:formatBytes(file.size),bytes:file.size,storedName:result.storedName,lastViewedAt:null,ocrStatus:'not-indexed',ocrText:'',linkedExpenseId:expenseId};
      state.documents.unshift(documentRecord);
      if(createExpense){state.expenses.unshift({id:expenseId,vehicleId:state.activeVehicleId,date:expenseDate,category:String(fd.get('Expense Category')||'Maintenance'),vendor:shop,services,notes:services.join(', ')||documentRecord.name,amount:expenseAmount,...(expenseCoverage==='None'?{}:{coverageType:expenseCoverage,coveredAmount}),linkedDocumentId:documentId,linkedDocumentStoredName:documentRecord.storedName,source:'document-upload',createdAt:now,updatedAt:now})}
