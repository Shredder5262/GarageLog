...
function settingsDeviceRows(){
  ...
  return settingsObdDevices.map(device=>{
    ...
    return `<article class="settings-obd-row">
      <div class="settings-obd-main"><strong>${esc(device.displayName||'GarageLog OBD')}</strong><code>${esc(device.deviceId)}</code><small>Last seen ${esc(settingsFormatDate(device.lastSeenUtc))}${device.lastVin?` · VIN ${esc(device.lastVin.slice(-8))}`:''}</small></div>
      ...
      <div class="settings-obd-status"><span class="settings-status ${device.vehicleId?'active':'expired'}">${device.vehicleId?'Associated':'Needs vehicle'}</span><small>${esc(device.vehicleName||'Choose a vehicle once; future trips will attach automatically.')}</small></div>
    </article>`;
  }).join('');
}
...
function settingsOdometerRows(){
  ...
  if(!actionable.length){
    return `<div class="settings-empty"><strong>No odometer updates waiting</strong><p>OBD mileage proposals will appear here. Manual GarageLog odometer readings remain authoritative.</p></div>`;
  }
    ...
    <section class="card settings-section">
      <div class="settings-section-heading">
        <div>
          ...
          <p>Associate each GarageLog OBD device once. VIN matching is automatic when available; otherwise the saved device pairing is used.</p>
        </div>
      </div>
      <div class="settings-security-note">${svg('shield')}<span>OBD trips never overwrite vehicle mileage automatically. Newer readings are proposed here so manual odometer entries can remain authoritative.</span></div>
